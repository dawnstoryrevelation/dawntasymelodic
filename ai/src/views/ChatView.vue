...
const simulateStreamingText = async (messageIndex, text, isReasoning = false) => {
  ...
  try {
    ...
    // Call server-side API for image generation instead of direct OpenAI call
    const response = await fetch("/api/openai-image", {
      ...
      body: JSON.stringify({
        prompt: promptText,
        model: "dall-e-3", // DALL-E-3 models
...
.message-stat-item svg {
  width: 12px;
  height: 12px;
  opacity: 0.7;
}

/* Fade-in animation for source cards */
@keyframes fadeInUp {
  from {
    opacity: 0;
